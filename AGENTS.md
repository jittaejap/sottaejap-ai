...
| 회고 표준 태그 · FR/NFR 번호                   | `sottaejap-docs/02_요구사항_정의서.md`                   |
| `/chat` DTO · `task_context.state` 구조 · Spring 내부 AI API 6종 · 타임아웃 | `sottaejap-docs/05_API_명세서.md` §3 |
...
- Tool Calling 실행 루프 · REFLECTION Handler · 금융 RAG 문서 적재는 TODO입니다. Agent는 지금 LLM 1회 호출 또는 폴백만 합니다.
  회고 추출기(`app/reflection/extractor.py`)는 구현되어 있지만 Handler가 없어 아직 `/chat` 경로에서 불리지 않습니다.
