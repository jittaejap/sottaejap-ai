...
| 축 | 무엇을 재나 | 기준 |
| --- | --- | --- |
| **재현율** | 문장에 분명히 있는 값을 뽑아냈나 | 기준선 대비 개선 |
...
### 실측 결과 (2026-09-09 · `gpt-4o-mini` · 29문장 · 5회 반복)

| | 개선 전 | 개선 후 |
| --- | --- | --- |
| 재현율 | 25~26/26 (96.2~100%) | **26/26 (100%) × 5회** |
| 과잉추론 | 2~3/90 항목 | **0/90 항목 × 5회** |
...
### 곁가지로 발견한 것 — INTRO 인사가 반말로 나갔다

같은 검증 중에 회고 **첫 화면** 인사가 반말로 나가는 것을 발견했다 (`"함께 돌아보자."` · `"발생했어."` — 4/4 재현). 02 NFR-09 · 01 E-90의 해요체 규칙 위반이다.

**원인**은 `build_system_prompt()`가 공통 `SYSTEM_PROMPT` **뒤에** Task 지시문을 붙이는 구조다. 나중에 붙은 쪽이 이기므로 앞의 해요체 규칙이 밀린다 — §10의 FINANCE_QA 길이 규칙이 깨졌던 것과 **같은 자리, 같은 함정**이다. 여기서는 지시문의 `"돌아보자고 권하는"`이라는 표현 자체가 문체 본보기로 샜다.

**조치와 실측**: 지시문에서 `"돌아보자고"`를 빼고 해요체 어미 규칙을 지시문 안에 직접 넣었다 (해요체 6/6). `temperature=0.2`도 함께 재봤으나 **0/6으로 효과가 없었다** — §10의 "문체·환각에는 temperature가 듣지 않는다"는 실측과 같다. 그래서 값을 배선하지 않고 지시문만 고쳤다.

**교훈**: Task 지시문을 새로 쓸 때는 `SYSTEM_PROMPT`의 톤 규칙이 지켜질 것이라 가정하지 않는다. 지시문에 쓰는 **예시 표현의 문체 자체가 출력 문체가 된다.**
