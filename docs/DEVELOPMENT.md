...
### 실측 결과 (2026-09-09 · `gpt-4o-mini` · 29문장)

| | 개선 전 | 개선 후 |
| --- | --- | --- |
| 재현율 | 25~26/26 (96.2~100%) | **26/26 (100%) × 10회** |
| 과잉추론 | 2~3/90 항목 | **0/90 항목 × 10회** |

개선 전에 5회 모두 재현된 과잉추론 두 가지가 이번 수정의 표적이다.
...
**규칙 하나하나를 빼서 다시 재봤다.** 목적 태그 매핑 문구 네 줄을 모두 빼고 5회를 재니 `"세일한다길래 계획에 없던 걸 홧김에 질렀어요"`만 5회 전부 `purpose=null`로 떨어졌고, 그중 2회는 `repeat_intention=false`까지 새어 나왔다 — 목적이 갈 곳을 잃자 후회하는 말이 반복 의향으로 흘렀다. `충동` 한 줄만 되살리자 10회 전부 26/26 · 0/90이 됐다. **`휴식·취미` · `자기계발` · `필수품` 세 줄은 없어도 재현율이 그대로다.** 그래서 프롬프트에는 실측이 요구한 두 줄(`식사`↔`만남·사교` 충돌, `충동`)만 남겼다.

`충동` 줄에는 우선순위 문장(`"다른 목적도 함께 걸리면 그 목적을 씁니다"`)을 함께 붙였다. 정본에 태그 우선순위가 없어서, `"치약이 떨어져서 계획 없이 샀다"`처럼 `필수품`과 겹치는 발화에 프롬프트가 답을 못 하면 안 되기 때문이다.

**한 회차만 보고 회귀로 단정하지 않는다.** 최종 프롬프트로 15회를 재는 동안 `"아니요"`(직전 질문이 반복 의향) 한 문장이 1회 `null`로 떨어졌다. 항목 단위로 389/390이다. 종료 코드는 한 회차라도 어긋나면 1이므로, 빨간 결과를 보면 **먼저 다시 돌려서** 매번 같은 문장이 같은 방향으로 틀리는지 확인한다. 매번 재현되면 규칙 문제이고, 한 번뿐이면 표집 흔들림이다.
...
**헤더에 "둘 이상 걸릴 때"라고 적는 것만으로는 부족하다.** `"즐기거나 쉬려고 한 것이면 휴식·취미"`처럼 발화를 한 태그로 **매핑하는 문장**은, 어느 헤더 아래에 있든 모델에게는 그 태그의 정의로 전달된다. 실제로 `필수품`과 `충동`을 나란히 정의해 두면 `"치약이 떨어져서 계획 없이 샀다"`가 둘 다에 걸리는데, 정본에 우선순위가 없어 프롬프트가 답할 수 없다. 그래서 **실측에서 실제로 충돌이 난 한 쌍(`식사` · `만남·사교`)과 null 복귀만** 남기고 나머지 매핑 문장은 뺐다. 태그 정의를 넣을지는 정본(01 §2) 결정이 먼저다.
