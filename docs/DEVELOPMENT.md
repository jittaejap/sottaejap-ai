...
**결론**: 프롬프트·생성 파라미터 조정만으로는 이 케이스를 완전히 막지 못했다. 다음으로 시도할 수단은 프롬프트가 아니라 **검색 단계의 유사도 score 하한**이다 — `SearchResult.score`(`1 - cosine distance`)에 최소 임계값을 두면, 관련은 있지만 답은 아닌 청크가 걸러져 `NO_EVIDENCE_INSTRUCTION` 경로가 살아나면서 이 사례와 위 "검색 최신성 충돌" 사례가 같은 지점에서 함께 완화될 가능성이 높다. 임계값은 실측이 필요해 이번 PR 범위에서는 다루지 않고 별도 이슈로 분리한다.

**후속 조치**: 검색 단계 score 하한 도입을 별도 이슈로 진행한다. 근거 조립 단계에서 "예를 들어" 패턴을 필터링하는 방법도 검토했으나, 가산금리·단리복리처럼 정상적인 예시 설명까지 걸러질 위험이 있어 score 하한보다 후순위로 둔다.
