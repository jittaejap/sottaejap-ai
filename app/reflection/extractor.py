...
- satisfaction은 좋았는지 아쉬웠는지를 **직접 말했을 때만** HIGH 또는 LOW입니다.
  예를 들어 "다신 안 살래요"는 repeat_intention이 false라는 뜻이고, satisfaction은
  "UNKNOWN"입니다. 만족했는지는 사용자에게 따로 묻습니다.
- repeat_intention은 다음 번에 어떻게 하겠다고 말했을 때, 또는 직전 질문이 반복 의향을
  물었고 거기에 답했을 때 정합니다. 줄이겠다 · 아껴야겠다 같은 다짐이나 후회하는 말은
  반복 의향의 답이 아니라 null입니다.
- 사람이 문장에 나와도 그 사람과 함께 있었다는 말이 없으면 companion은 null입니다.
  가게를 추천해 준 사람, 장소를 설명하려고 언급한 사람은 동행인이 아닙니다.

목적을 고르는 규칙 (둘 이상 걸릴 때 씁니다):
- 사람을 만나는 것 자체가 그 자리의 목적이면 "만남·사교", 함께 먹은 것뿐이면 "식사"입니다.
- 즐기거나 쉬려고 한 것이면 "휴식·취미", 배우거나 실력을 늘리려는 것이면 "자기계발"입니다.
- 없으면 생활이 안 되는 것을 채운 소비면 "필수품"입니다.
- 계획에 없었다거나 즉흥적으로 샀다고 말했으면 "충동"입니다.
- 어느 것도 걸리지 않으면 purpose는 null입니다. 고르기 어렵다고 "기타"를 쓰지 않습니다.
